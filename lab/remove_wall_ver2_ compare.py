...
def remove_small_floating_clusters(pcd, floor_height):
    ...
    print(f"[INFO] removed floating small clusters: {len(removed_pcd.points):,}")
...
def main():
    ...
    if REMOVE_SMALL_FLOATING_CLUSTERS:
        (
            final_objects,
            removed_floating_clusters
        ) = remove_small_floating_clusters(
            objects_after_small_fragment_cleanup,
            floor_height
        )
    else:
        final_objects = objects_after_small_fragment_cleanup
        removed_floating_clusters = o3d.geometry.PointCloud()
    ...
    # 9) 시각화
    # 회색: 최종 남은 객체 후보
    # 빨간색: 공중에 떠 있어서 제거된 작은 클러스터
    final_vis = o3d.geometry.PointCloud(final_objects)
    removed_vis = o3d.geometry.PointCloud(removed_floating_clusters)

    final_vis.paint_uniform_color([0.6, 0.6, 0.6])
    removed_vis.paint_uniform_color([1.0, 0.0, 0.0])

    print("\n[VIEW]")
    print("회색: 최종 남은 객체 후보")
    print("빨간색: 공중에 떠 있어서 제거된 작은 클러스터")

    if len(removed_vis.points) > 0:
        o3d.visualization.draw_geometries([final_vis, removed_vis])
    else:
        o3d.visualization.draw_geometries([final_vis])
