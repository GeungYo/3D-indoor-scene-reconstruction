...
MESH_NORMAL_RADIUS = 0.12
MESH_NORMAL_MAX_NN = 100
MESH_ORIENT_K = 50
...
MESH_MAX_POINT_DISTANCE = 0.025

# 빈 공간을 가로지르는 큰 삼각형 제거
# 입력 포인트 간격이 약 0.02m이므로 0.05m부터 시작
MESH_MAX_TRIANGLE_EDGE = 0.06

# 삼각형 중심이 원본 Point Cloud에서 멀면 가짜 면으로 판단
MESH_MAX_TRIANGLE_CENTER_DISTANCE = 0.025

# 작은 분리 조각 제거 기준
MESH_MIN_COMPONENT_TRIANGLES = 100

# 최종 Mesh 표면 부드럽게 만들기
MESH_SMOOTH_ITERATIONS = 50
...
def pointcloud_to_mesh(cluster, cluster_label):
    ...
    if len(mesh.triangles) > 0:
        ...
        center_pcd.points = o3d.utility.Vector3dVector(triangle_centers)

        center_distances = np.asarray(
            center_pcd.compute_point_cloud_distance(mesh_input)
        )

        bad_triangle_mask = (
            (max_edge > MESH_MAX_TRIANGLE_EDGE) |
            (
                center_distances >
                MESH_MAX_TRIANGLE_CENTER_DISTANCE
            )
        )

        removed_bad_triangles = int(np.sum(bad_triangle_mask))
        mesh.remove_triangles_by_mask(bad_triangle_mask)
        mesh.remove_unreferenced_vertices()

        print(
            f"[MESH CLEAN] cluster {cluster_label}: "
            f"긴 면/빈 공간 면 {removed_bad_triangles:,}개 제거"
        )
    ...
    # 아주 작은 분리 조각 제거
    if len(mesh.triangles) > 0 and MESH_MIN_COMPONENT_TRIANGLES > 0:
        ...
        small_component_mask = np.array([
            cluster_counts[cluster_id] < MESH_MIN_COMPONENT_TRIANGLES
            for cluster_id in triangle_clusters
        ])

        removed_small_triangles = int(np.sum(small_component_mask))
        mesh.remove_triangles_by_mask(small_component_mask)
        mesh.remove_unreferenced_vertices()

        print(
            f"[MESH CLEAN] cluster {cluster_label}: "
            f"작은 분리 조각 면 {removed_small_triangles:,}개 제거"
        )
    ...
    if MESH_SMOOTH_ITERATIONS > 0:
        mesh = mesh.filter_smooth_taubin(
            number_of_iterations=MESH_SMOOTH_ITERATIONS,
            lambda_filter=MESH_SMOOTH_LAMBDA,
            mu=MESH_SMOOTH_MU
        )
        mesh.remove_unreferenced_vertices()
